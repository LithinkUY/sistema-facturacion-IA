...
// WhatsApp Webhook (público, sin auth - Meta necesita acceder)
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    Route::post('/products/save_manual_product', [ProductController::class, 'saveManualProduct']);
    ...
    Route::get('payment-receipts/{payment_receipt}/print', [PaymentReceiptController::class, 'print'])->name('payment-receipts.print');
    Route::resource('payment-receipts', PaymentReceiptController::class)->only([
        'index',
        'create',
        'store',
        'show',
    ...
    // Tareas de órdenes
    Route::post('order-tasks', [OrderTaskController::class, 'store'])->name('order-tasks.store');
    Route::put('order-tasks/{id}', [OrderTaskController::class, 'update'])->name('order-tasks.update');
    Route::post('order-tasks/{id}/status', [OrderTaskController::class, 'updateStatus'])->name('order-tasks.update-status');
    Route::delete('order-tasks/{id}', [OrderTaskController::class, 'destroy'])->name('order-tasks.destroy');
    Route::post('order-tasks/{id}/checklist', [OrderTaskController::class, 'addChecklist'])->name('order-tasks.add-checklist');
    Route::post('order-tasks/checklist/{id}/toggle', [OrderTaskController::class, 'toggleChecklist'])->name('order-tasks.toggle-checklist');
    Route::delete('order-tasks/checklist/{id}', [OrderTaskController::class, 'deleteChecklist'])->name('order-tasks.delete-checklist');
    Route::post('order-tasks/{id}/comment', [OrderTaskController::class, 'addComment'])->name('order-tasks.add-comment');

    // Asistente IA (Gemini)
    Route::get('ai-agent', [AiAgentController::class, 'index'])->name('ai-agent.index');
    Route::post('ai-agent/send', [AiAgentController::class, 'sendMessage'])->name('ai-agent.send');
    Route::get('ai-agent/history', [AiAgentController::class, 'getHistory'])->name('ai-agent.history');
    Route::post('ai-agent/new-session', [AiAgentController::class, 'newSession'])->name('ai-agent.new-session');
    Route::post('ai-agent/delete-session', [AiAgentController::class, 'deleteSession'])->name('ai-agent.delete-session');
    Route::get('ai-agent/settings', [AiAgentController::class, 'settings'])->name('ai-agent.settings');
    Route::post('ai-agent/save-settings', [AiAgentController::class, 'saveSettings'])->name('ai-agent.save-settings');

    // WhatsApp Business (panel admin)
    ...
    Route::get('whatsapp/detect-ngrok', [WhatsAppController::class, 'detectNgrok'])->name('whatsapp.detect-ngrok');
