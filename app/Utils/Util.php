    ...
    */
    ...
    public function allModulesEnabled($business_id = null)
    {
        ...
        if (session()->has('business')) {
            ...
            $enabled_modules = $business_session['enabled_modules'] ?? null;
        }
        ...
        // Ensure enabled_modules is always an array (may come as JSON string from session/DB)
    ...
    {
        // Only log activity if subject is an Eloquent Model (required by Spatie ActivityLog)
        if (! ($on instanceof \Illuminate\Database\Eloquent\Model)) {
            return;
        }
        ...
        if ($business && !empty($business->time_zone)) {
            date_default_timezone_set($business->time_zone);
        } else {
            // Fallback a una zona horaria por defecto si no hay business
