...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            // Ensure enabled_modules is always an array (may come as JSON string from session)
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['purchase.view', 'purchase.create', 'sell.view', 'sell.create'])) {
                $menu->dropdown(
                    'Órdenes de Pedido',
                    function ($sub) {
                        $sub->url(
                            action([\App\Http\Controllers\OrderPedidoController::class, 'index']),
                            'Listado de Órdenes',
                            ['icon' => '', 'active' => request()->segment(1) == 'order-pedidos' && request()->segment(2) == null]
                        );
                        $sub->url(
                            action([\App\Http\Controllers\OrderPedidoController::class, 'create']),
                            'Nueva Orden',
                            ['icon' => '', 'active' => request()->segment(1) == 'order-pedidos' && request()->segment(2) == 'create']
                        );
                        $sub->url(
                            url('/order-pedidos/my-tasks'),
                            'Mis Tareas',
                            ['icon' => '', 'active' => request()->segment(1) == 'order-pedidos' && request()->segment(2) == 'my-tasks']
                        );
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
            ...
            $menu->url(
                url('/ai-agent'),
                'Asistente IA',
                ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 21v-4a3 3 0 0 1 3 -3h5" /><path d="M9 17l3 -3l-3 -3" /><path d="M14 3v4a1 1 0 0 0 1 1h4" /><path d="M5 11v-6a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2h-1" /><circle cx="19" cy="17" r="3" /></svg>', 'active' => request()->segment(1) == 'ai-agent']
            )->order(82);
            ...
            $menu->url(
                url('/whatsapp'),
                'WhatsApp',
                ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 21l1.65 -3.8a9 9 0 1 1 3.4 2.9l-5.05 .9" /><path d="M9 10a.5 .5 0 0 0 1 0v-1a.5 .5 0 0 0 -1 0v1a5 5 0 0 0 5 5h1a.5 .5 0 0 0 0 -1h-1a.5 .5 0 0 0 0 1" /></svg>', 'active' => request()->segment(1) == 'whatsapp']
            )->order(83);
