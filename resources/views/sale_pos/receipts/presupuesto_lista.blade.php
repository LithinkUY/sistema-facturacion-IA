...
<div class="presupuesto-container">
        ...
        <div class="presupuesto-header-left">
            <div class="presupuesto-title">
                ...
                @else
                    Presupuesto
                @endif
                <small style="font-size: 14px; color: #666; font-weight: 400;">- Cotización Unificada</small>
            </div>
    ...
    {{-- ===== PRODUCT TABLE - LISTA CON PRECIO REGULAR, OFERTA Y AHORRO ===== --}}
    @php
        $total_ahorro = 0;
    @endphp
    <table class="presupuesto-table">
        <thead>
            <tr>
                ...
                <th style="width: 14%; text-align: right;">Precio Regular</th>
                <th style="width: 14%; text-align: right;">Precio Oferta</th>
                <th style="width: 14%; text-align: right;">Total</th>
                <th style="width: 13%; text-align: right;">Ahorro</th>
            </tr>
        </thead>
        <tbody>
            @foreach($receipt_details->lines as $line)
                @php
                    // Precio regular (antes de descuento) por unidad
                    $precio_regular_uf = $line['unit_price_before_discount_uf'] ?? 0;
                    // Precio con descuento (oferta) por unidad - sin impuesto
                    $precio_oferta_uf = $line['unit_price_uf'] ?? $precio_regular_uf;
                    // Cantidad
                    $qty = $line['quantity_uf'] ?? 1;
                    // Total de la línea (precio oferta * cantidad)
                    $line_total_uf = $line['line_total_exc_tax_uf'] ?? ($precio_oferta_uf * $qty);
                    // Ahorro por línea = (precio_regular - precio_oferta) * cantidad
                    $ahorro_linea = ($precio_regular_uf - $precio_oferta_uf) * $qty;
                    if ($ahorro_linea < 0) $ahorro_linea = 0;
                    $total_ahorro += $ahorro_linea;
                    // Si no hay descuento, precio oferta = precio regular
                    $tiene_descuento = $ahorro_linea > 0;
                @endphp
                <tr>
                    ...
                    <td style="text-align: right;">
                        @if($tiene_descuento)
                            <span class="precio-tachado">{{ $line['unit_price_before_discount'] }}</span>
                        @else
                    ...
                    <td style="text-align: right;">
                        @if($tiene_descuento)
                            <span class="precio-oferta">{{ $line['unit_price_exc_tax'] }}</span>
                            @if(!empty($line['line_discount_percent']))
                                <br><small style="color: #27ae60;">-{{ $line['line_discount_percent'] }}%</small>
                            @endif
                        @else
                    ...
                    <td style="text-align: right;">
                        @if($tiene_descuento)
                            <span class="precio-oferta">{{ number_format($ahorro_linea, 2) }}</span>
                        @else
    ...
    @if($total_ahorro > 0)
        <div class="ahorro-badge">
            <span>✓ Ahorro total en este presupuesto: {{ number_format($total_ahorro, 2) }}</span>
        </div>
    @endif
